...
class ConvEncoder(tools.Module):
    ...
    def __call__(self, obs) -> Any:
        ...
        shape = tf.concat([tf.shape(obs['image'])[-3:], [32 * self._depth]], 0)  # why?
        return tf.reshape(x, shape) ## check this dimension
...
class ConvDecoder(tools.Module):
    ...
    def __call__(self, features) -> Any:
        ...
        return tfd.Independent(tfd.Normal(mean, 1), len(self._shape))           # ??? Not understood