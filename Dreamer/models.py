...
class RSSM(tools.Module): 
    ...
    def observe(self, embed, action, state=None):
        ...
        embed = tf.transpose(embed, [1, 0, 2])      # check the shape
        action = tf.transpose(action, [1, 0, 2])    # why this?
    ...
    def obs_step(self, prev_state, prev_action, embed):
        ...
        stoch = self._get_dist({'mean': mean, 'std': std}).sample()
...
class ConvEncoder(tools.Module):
    ...
    def __call__(self, obs) -> Any:
        ...
        x = tf.reshape(obs['image'], (-1) + tuple(obs['image'].shape[-3:])) # expand_dims
        ...
        shape = tf.concat([tf.shape(obs['image'])[-3:], [32 * self._depth]], 0)  # N, H, W, C
        return tf.reshape(x, shape) ## check this dimension (N, H, W, C)
...
class ConvDecoder(tools.Module):
    ...
    def __call__(self, features) -> Any:
        ...
        mean = tf.reshape(x, tf.concat([tf.shape(features)[:-1], self._shape], 0))  # Get batch size from features and rest from shape =  (N, 64, 64, 3)
...
class DenseDecoder(tools.Module):
    ...
    def __call__(self, features) -> Any:
        ...
        x = tf.reshape(x, tf.concat([tf.shape(features)[:-1], self._shape], 0))  # N, H, W, C
...
class ActionDecoder(tools.Module):
    def __init__(self,
                 size, layers, units, dist='tanh_normal', act=tf.nn.elu,
    ...
    def __call__(self, features):
        ...
        if self._dist == 'tanh_normal':
            ...
            dist = tfd.TransformedDistribution(dist, tools.TanhBijector()) # not understood
