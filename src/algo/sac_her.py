...
class SACHERActor:
    ...
    def _build_net(self):
        ...
        if self.feature_model is None:
            f_state = tf.keras.layers.Dense(
                128, activation="relu")(state_input)
        ...
        model = tf.keras.Model(inputs=[state_input, goal_input], outputs=[
                               mu, log_sig], name='actor')
        model.summary()
    ...
    def policy(self, state, goal):
        ...
        log_pi_a = tf.reduce_sum(
            (log_pi_ - tf.math.log(1 - action ** 2 + 1e-16)), axis=-1, keepdims=True)
...
class SACHERCritic:
    ...
    def _build_net(self):
        ...
        model = tf.keras.Model(
            inputs=[state_input, goal_input, action_input], outputs=q_value)
        model.summary()
    ...
    def train(self, state_batch, action_batch, goal_batch, y):
        with tf.GradientTape() as tape:
...
class SACHERAgent:
    def __init__(self, state_size, action_size, upper_bound,
        ...
        self.target_entropy = - \
            tf.constant(np.prod(self.action_size), dtype=tf.float32)
        self.gamma = gamma                  # discount factor
        ...
        if self.image_input:        # input is an image
            ...
            self.feature = FeatureNetwork(
                self.state_size, self.use_attention, self.lr_a)
    ...
    def sample_action(self, state, goal):
        ...
        if state.ndim < len(self.state_size) + 1:      # single sample
            ...
            tf_goal = tf.expand_dims(
                tf.convert_to_tensor(goal, dtype=tf.float32), 0)
        else:       # for a batch of samples
            tf_state = tf.convert_to_tensor(state, dtype=tf.float32)
            tf_goal = tf.convert_to_tensor(goal, dtype=tf.float32)

        action, log_pi = self.actor.policy(
            tf_state, tf_goal)  # returns tensors
    ...
    def update_target_networks(self):
        for theta_target, theta in zip(self.target_critic1.model.trainable_variables,
            ...
            theta_target = self.polyak * \
                theta_target + (1 - self.polyak) * theta
    ...
    def update_q_networks(self, states, actions, rewards, next_states, dones, goals):
        ...
        c1_loss = self.critic1.train(states, goals, actions, y)
        c2_loss = self.critic2.train(states, goals, actions, y)
    ...
    def train(self):
        ...
        for _ in range(self.epochs):
            ...
            critic_loss = self.update_q_networks(
                states, actions, rewards, next_states, dones, goals)

            # update actor
            actor_loss = self.actor.train(
                states, self.alpha, self.critic1, self.critic2, goals)
    ...
    def run(self, env, seasons=50, training_batch=1024, train_freq=20,
        ...
        for s in range(seasons):
            ...
            for t in range(training_batch):
                ...
                if done:
                    ...
                    if self.her_strategy == 'final':    # final state strategy
                        ...
                        raise ValueError(
                            'Invalid choice for HER strategy. Exiting ..')

                    self.add_her_experience(
                        ep_experience, hind_goal, extract_feature=False)
            ...
            if mean_s_score > best_score:
                ...
                print('Season: {}, Update best score: {}-->{}, Model saved!'.format(s,
            ...
            if success_value is not None:
                if best_score > self.success_value:
                    print('Problem is solved in {} episodes with score {}'.format(
                        s, best_score))
        ...
        env.close()
        print('Mean episodic score over {} episodes: {:.2f}'.format(
            self.episodes, np.mean(ep_scores)))

    def her_reward_func_1(self, state, goal, thr=0.3):
        tf_state = tf.expand_dims(tf.convert_to_tensor(
            state, dtype=tf.float32), axis=0)
        tf_goal = tf.expand_dims(tf.convert_to_tensor(
            goal, dtype=tf.float32), axis=0)
