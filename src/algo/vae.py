...
Updates: December 11, 2021
- This is slightly different from 'VariationAutoEncoder.py' file in the common folder.
- It is possible to change the number of conv_layers and dense_layers through **kwargs arguments.
- updated viz_latent_space() function to use t-SNE plot
"""
import os
import numpy as np
...
class Encoder(keras.Model):
    def __init__(self, input_shape, latent_dim, **kwargs) -> None:
        ...
        self.dense_layers = kwargs.get('dense_layers', [20, 10])    # excluding latent dimension
    ...
    def _build_model(self):
        inp = tf.keras.Input(shape=self.inp_shape, name='encoder_input')
        cx = inp
        for i in range(len(self.conv_layers)):
            cx = Conv2D(filters=self.conv_layers[i], kernel_size=self.kernel_size, 
                                strides=self.strides, padding=self.padding, 
                                activation='relu',
                                name='conv_{}'.format(i))(cx)
            cx = BatchNormalization()(cx)

        x = Flatten()(cx)

        for i in range(len(self.dense_layers)):
            x = Dense(self.dense_layers[i], activation='relu', name='dense_{}'.format(i))(x)
    ...
    def get_last_conv_shape(self):
        ...
        last_conv_layer_name = 'conv_' + str(len(self.conv_layers)-1)
    ...
    def viz_latent_space(self, data):
        ...
        tsne_result=tsne.fit_transform(mu)

        plt.figure(figsize=(8, 10))
        plt.scatter(tsne_result[:, 0], tsne_result[:, 1], c=target_data)
        plt.xlabel('z - dim 1')
        plt.ylabel('z - dim 2')
        plt.colorbar()
...
class Decoder(keras.Model):
    def __init__(self, out_shape, conv_shape, latent_dim, **kwargs) -> None:
        ...
        self.conv_layers = kwargs.get('conv_layers', [32, 64,])
        self.kernel_size = kwargs.get('kernel_size', 3)
        self.dense_layers = kwargs.get('dense_layers', [10, 20])    # excluding latent dimension
        self.strides = kwargs.get('strides', 2)
        self.padding = kwargs.get('padding', 'same')
    ...
    def _build_model(self):
        # Definition
        d_inp = Input(shape=(self.latent_dim, ), name='decoder_input')
        x = d_inp
        for i in range(len(self.dense_layers)):
            x = Dense(self.dense_layers[i], activation='relu')(x)
            x = BatchNormalization()(x)
        x = Dense(self.conv_shape[1] * self.conv_shape[2] * self.conv_shape[3], 
                                    activation='relu')(x)
        x = BatchNormalization()(x)
        x = Reshape((self.conv_shape[1], self.conv_shape[2], self.conv_shape[3]))(x)

        for i in range(len(self.conv_layers)):
            x = Conv2DTranspose(filters=self.conv_layers[i], 
                                kernel_size=self.kernel_size, 
                                strides=self.strides, 
                                padding=self.padding, activation='relu')(x)
            x = BatchNormalization()(x)
        outp = Conv2DTranspose(filters=self.num_channels, 
                    kernel_size=self.kernel_size, activation='sigmoid', 
                    padding=self.padding, name='decoder_output')(x)
        decoder = Model(d_inp, outp, name='decoder')
        decoder.summary()
        return decoder
...
if __name__ == '__main__':
    ...
    print('MNIST digit label shape:', np.shape(mnist_labels))
