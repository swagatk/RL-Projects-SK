...
# Local imports
from common.FeatureNet import FeatureNetwork, CNNLSTMFeatureNetwork 
...
class IPGAgent:
        ...
        for ep in range(max_eps):
            ...
            while True:
                ...
                if self.cnn_lstm is not None:
                    ...
                    action = self.policy(state_stack, deterministic=False)
                else:
    ...
    def run(self):
        ...
        # create buffer for lstm network
        if self.cnn_lstm is not None:
            state_buffer = []
            next_state_buffer = []
        ...
        for s in range(self.SEASONS):
            ...
            for _ in range(self.training_batch):    # time steps
                ...
                if self.cnn_lstm is not None:
                    state_buffer.append(state)
                    state_stack = self.prepare_input(state_buffer)
                    action = self.policy(state_stack)
                else:
                ...
                if self.cnn_lstm is not None:
                    self.buffer.record([state_stack, action, reward, next_state_stack, done])
                else:
