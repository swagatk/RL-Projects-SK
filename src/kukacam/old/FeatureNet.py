...
class FeatureNetwork:
    def __init__(self, state_size, attn: dict = None, learning_rate=1e-3):
        ...
        #self.model = self._build_net_RC() # for Racecar
        #self.model = self._build_net2() # bigger network with pooling layers
    ...
    def _build_net_RC(self, conv_layers=[32, 64, 64, 32], dense_layers=[256, 128, 128, 64]):
        img_input = tf.keras.layers.Input(shape=self.state_size)
